...
test("project-input load failures recover automatically instead of escalating to Owner", async () => {
  const runtime = await fs.readFile(
    new URL("../src/runtime/brain-worker-cli.mjs", import.meta.url),
    "utf8"
  );

  assert.match(runtime, /class ProjectInputLoadError extends Error/);
  assert.match(runtime, /PROJECT_INPUT_LOAD_RETRY/);
  assert.match(runtime, /status: "RECOVERING"/);
  assert.match(runtime, /project input temporarily unavailable; retrying automatically/);
