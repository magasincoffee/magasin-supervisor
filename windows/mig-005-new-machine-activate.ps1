...
function Assert-GitSha([string]$Sha,[string]$Label) {
    if ($Sha -notmatch '^[a-f0-9]{40}$') { throw "$Label must be an exact 40-character git SHA." }
}

function Assert-GitCandidate([string]$Sha) {
    Assert-GitSha -Sha $Sha -Label 'CandidateSha'
...
function Get-PackageManifestCandidate([string]$ZipPath) {
    Add-Type -AssemblyName System.IO.Compression.FileSystem
    $zip = [IO.Compression.ZipFile]::OpenRead($ZipPath)
    try {
        $entry = $zip.Entries | Where-Object { $_.FullName -eq 'MIG_005_STATE_TRANSFER_MANIFEST.json' } | Select-Object -First 1
        if (-not $entry) { throw 'MIG-005 package transfer manifest is missing.' }
        ...
        if ([string]$manifest.schema_version -ne 'supervisor-mig005-state-transfer.v1') {
            throw 'MIG-005 package transfer manifest schema mismatch.'
        }
        return [string]$manifest.candidate_sha
    } finally {
        $zip.Dispose()
    }
}

function Assert-PackageRuntimeProvenance([string]$PackageSha,[string]$RuntimeSha) {
    Assert-GitSha -Sha $PackageSha -Label 'PackageCandidateSha'
    Assert-GitSha -Sha $RuntimeSha -Label 'CandidateSha'

    & git -C $repoRoot cat-file -e ($PackageSha + '^{commit}') 2>$null
    if ($LASTEXITCODE -ne 0) { throw 'PackageCandidateSha does not exist in local git.' }

    & git -C $repoRoot merge-base --is-ancestor $PackageSha $RuntimeSha 2>$null
    if ($LASTEXITCODE -ne 0) { throw 'PackageCandidateSha is not an ancestor of CandidateSha.' }

    $transferPath = 'windows/mig-005-state-transfer.ps1'
    $packageBlob = (& git -C $repoRoot rev-parse ($PackageSha + ':' + $transferPath)).Trim()
    if ($LASTEXITCODE -ne 0 -or $packageBlob -notmatch '^[a-f0-9]{40}$') {
        throw 'Package candidate transfer blob cannot be resolved.'
    }
    $runtimeBlob = (& git -C $repoRoot rev-parse ($RuntimeSha + ':' + $transferPath)).Trim()
    if ($LASTEXITCODE -ne 0 -or $runtimeBlob -notmatch '^[a-f0-9]{40}$') {
        throw 'Runtime candidate transfer blob cannot be resolved.'
    }
    if ($packageBlob -ne $runtimeBlob) {
        throw 'Package/runtime transfer blob identity mismatch.'
    }

    Write-Host 'MIG_005_PACKAGE_CANDIDATE_SHA_VERIFIED=True'
    Write-Host 'MIG_005_RUNTIME_CANDIDATE_SHA_VERIFIED=True'
    Write-Host 'MIG_005_PACKAGE_RUNTIME_ANCESTRY_VERIFIED=True'
    Write-Host 'MIG_005_TRANSFER_BLOB_IDENTITY_VERIFIED=True'
    Write-Host 'MIG_005_PACKAGE_RUNTIME_PROVENANCE_COMPATIBLE=True'
}
...
if (-not $PackageZip -or -not $ExpectedPackageSha256 -or -not $PackageCandidateSha -or -not $CandidateSha) { throw 'Activate/Validate requires PackageZip, ExpectedPackageSha256, PackageCandidateSha and CandidateSha.' }
...
Assert-GitSha -Sha $PackageCandidateSha -Label 'PackageCandidateSha'
...
$manifestCandidateSha = Get-PackageManifestCandidate -ZipPath $PackageZip
if ($manifestCandidateSha -ne $PackageCandidateSha) { throw 'Package manifest candidate SHA does not match PackageCandidateSha.' }
Assert-PackageRuntimeProvenance -PackageSha $PackageCandidateSha -RuntimeSha $CandidateSha
...
try {
    & powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File $transfer -Mode Import -DestinationRoot $destinationRoot -PackageZip $PackageZip -ExpectedPackageSha256 $ExpectedPackageSha256 -CandidateSha $PackageCandidateSha
    if ($LASTEXITCODE -ne 0) { throw 'MIG-005 new-machine state Import failed.' }
    $importCompleted = $true

    & powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File $transfer -Mode Verify -DestinationRoot $destinationRoot -PackageZip $PackageZip -ExpectedPackageSha256 $ExpectedPackageSha256 -CandidateSha $PackageCandidateSha
