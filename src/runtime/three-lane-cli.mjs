...
import { ACTIONS, OBSERVATIONS } from "../decision.mjs";
import {
...
async function applyBrainVerdictDirective({
  lane,
  registryLane,
  directive,
  registry,
  ...
  await emitLaneEvent({
    ...
    actor: "BRAIN",
    event_type: transition.record.verdict === "ACCEPT"
      ? LANE_EVENT_TYPES.BRAIN_RESULT_ACCEPTED
      : LANE_EVENT_TYPES.BRAIN_RESULT_REJECTED,
    task_id: transition.record.task_id,
    phase: transition.record.verdict === "ACCEPT" ? "ACCEPTED" : "REJECTED",
    reason_code: transition.record.reason_code || undefined,
    relay_id: transition.record.relay_id,
    work_generation: Number(registryLane.work_generation || 0)
  });
  return transition;
}
...
async function adoptExistingBrainDirective({
  ...
  // Never re-adopt a directive that this durable lane has already consumed.
  // This matters after recovery/reset: brain_request_sent can be re-armed while
  // the latest visible assistant turn is still the previous directive.
  if (candidate.digest === registryLane.last_brain_directive_digest) {
    return null;
  }

  // previous_result is only authoritative when it still correlates with the
  // durable task/result pair. A stale Brain turn must not become the handshake
  // anchor, otherwise the lane can loop forever on the same correlation error.
  if (candidate.previous_result) {
    try {
...
async function dispatchWork({
  ...
  const planningContract = existingDispatchLatch
    ? Number(existingDispatchLatch.planning_contract_version || 0) >= 1
    : true;
...
async function executeWatchdogContinue({
  ...
  try {
    ...
    const preProbe = await assertConversationSafe(adapter, workPage, {
      brain: false,
      allowFull: true
    });
    ...
    const intentAt = new Date().toISOString();
    registryLane.work_watchdog = beginWatchdogContinueIntent(
      registryLane.work_watchdog,
      { now: intentAt }
    );
    await atomicJsonWrite(registryPath, registry);
    ...
    await safeLog(logPath, {
      type: "WORK_WATCHDOG_CONTINUE_INTENT",
      laneId: lane.lane_id,
      taskId: registryLane.task_id,
      reason: `epoch=${registryLane.work_watchdog.recovery_epoch};continue=1`
    });
    ...
    actionResult = await executeDecision({
      ...
      decision: {
        ...
        instruction: WATCHDOG_CONTINUE_INSTRUCTION
      },
      dryRun: false
    });
    ...
    if (!actionResult?.executed) {
      await safeLog(logPath, {
        type: "WORK_WATCHDOG_CONTINUE_ACTION_UNCERTAIN",
        laneId: lane.lane_id,
        taskId: registryLane.task_id,
        target: actionResult?.target || null,
        rejectionClass: actionResult?.rejection_class || null,
        reason: String(actionResult?.reason || "continue action did not execute").slice(0, 180)
      });
      return {
    ...
    registryLane.work_watchdog = markWatchdogContinued(
      registryLane.work_watchdog,
      { now: continuedAt }
    );
    await atomicJsonWrite(registryPath, registry);

    const reasonCode = actionResult.target === "SAFE_CONTINUE_CONTROL"
      ? "WATCHDOG_CONTINUE_CONTROL_CLICKED"
      : "WATCHDOG_CONTINUE_SENT";
    await emitLaneEvent({
      timestamp: continuedAt,
      lane_id: lane.lane_id,
      actor: "SUPERVISOR",
      event_type: LANE_EVENT_TYPES.WATCHDOG_CONTINUE_POKE,
      task_id: registryLane.task_id,
      phase: "CONTINUE_POKE",
      reason_code: reasonCode,
      work_generation: Number(registryLane.work_generation || 0),
      work_url_revision: Number(registryLane.applied_work_url_revision || 0),
      ...timingEventFields(registryLane.task_timing, continuedAt)
    });
    await safeLog(logPath, {
      type: "WORK_WATCHDOG_CONTINUE_EXECUTED",
      laneId: lane.lane_id,
      taskId: registryLane.task_id,
  ...
  registryLane.work_watchdog = markWatchdogPostContinueProbe(
    registryLane.work_watchdog,
    { now: probedAt }
  );
  await atomicJsonWrite(registryPath, registry);
...
async function processLaneTurn({
  ...
  if (registryLane.awaiting_work) {
    ...
    if (
      ...
      watchdogDecision.decision === WORK_WATCHDOG_DECISIONS.CONTINUE_UNCERTAIN
    ) {
      let recoveryMessage = "Work vẫn long-running; watchdog đang trong reload cooldown, không reload.";
      if (watchdogDecision.decision === WORK_WATCHDOG_DECISIONS.RECOVERY_UNCERTAIN) {
        recoveryMessage = "Watchdog reload intent đã persist; không tự replay reload sau restart/crash.";
      } else if (watchdogDecision.decision === WORK_WATCHDOG_DECISIONS.CONTINUE_UNCERTAIN) {
        recoveryMessage = "Watchdog continue intent đã persist; không tự gửi lại 'Tiếp tục thực hiện' sau restart/crash.";
      }
      return laneStatus(
        lane,
        registryLane,
    ...
    if (watchdogDecision.decision === WORK_WATCHDOG_DECISIONS.CONTINUE_ELIGIBLE) {
      if (!execute) {
        return laneStatus(
          lane,
          registryLane,
          "WORKING_LONG",
          "Watchdog đủ điều kiện gửi một continue poke nhưng runtime đang dry-run; không mutation.",
          {
            watchdog_phase: registryLane.work_watchdog.phase,
            task_elapsed_ms: watchdogDecision.elapsed_ms
          }
      ...
      const continuation = await executeWatchdogContinue({
        adapter,
        lane,
        registryLane,
        registry,
      ...
      if (continuation.status === "OWNER_STOP") {
        return laneStatus(
          lane,
          registryLane,
          "STOPPED",
          "Owner STOP/lane disable đã thắng race trước watchdog continue; không mutation."
        );
      }
      if (continuation.status === "MUTATION_BUSY") {
        return laneStatus(
          lane,
          registryLane,
          "WORKING_LONG",
          "Global mutation lease đang bận; watchdog continue yield và thử lại ở turn sau."
        );
      }
      if (continuation.status === "IDENTITY_MISMATCH") {
        return laneStatus(
          lane,
          registryLane,
          "WAIT_OWNER",
          "Watchdog huỷ continue vì exact task/Work/generation thay đổi trước mutation."
        );
      }
      if (continuation.status === "TARGET_QUARANTINED") {
        return laneStatus(
          lane,
          registryLane,
          "WAIT_OWNER",
          "Work target đang bị quarantine; watchdog không gửi continue."
        );
      }
      if (continuation.status === "SECURITY_BLOCKED") {
        return laneStatus(
          lane,
          registryLane,
          "WAIT_OWNER",
          "Work chat đang ở security/auth boundary; watchdog không gửi continue."
        );
      }
      if (continuation.status === "MARKER_MISSING") {
        return laneStatus(
          lane,
          registryLane,
          "POSSIBLY_STALLED",
          "Watchdog không xác minh được dispatch marker trên exact Work; không gửi continue."
        );
      }
      if (continuation.status === "PROGRESS_RESUMED") {
        return laneStatus(
          lane,
          registryLane,
          "WORKING_LONG",
      ...
      if (continuation.status === "ACTION_UNCERTAIN") {
        return laneStatus(
          ...
          registryLane,
          "WORKING_LONG",
          "Continue intent đã persist nhưng UI action không được xác nhận; watchdog fail-closed và không tự replay."
        );
      }

      const postObservation = buildSafeWorkObservation(
        continuation.probe.snapshot,
        continuation.probe.classification.observation === OBSERVATIONS.RESPONSE_COMPLETE
      );
      ...
      if (postActivity.baseline_initialized || postActivity.changed) {
        ...
        if (postActivity.event_due) {
          await emitLaneEvent({
            ...
            work_generation: Number(registryLane.work_generation || 0),
            ...timingEventFields(timing, continuation.at)
          });
        }
      }
      ...
      return laneStatus(
        ...
        watchdogDecision.decision === WORK_WATCHDOG_DECISIONS.WORKING_LONG
          ? "Watchdog continue đã kích hoạt và Work có fresh progress/running evidence."
          : "Watchdog đã gửi/click continue đúng một lần; đang observation, không gửi lại.",
        {
          watchdog_phase: registryLane.work_watchdog.phase,
          task_elapsed_ms: watchdogDecision.elapsed_ms,
          last_activity_at: timing.last_activity_at
