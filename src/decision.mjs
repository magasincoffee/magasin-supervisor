...
export const CANONICAL_CONTINUE_INSTRUCTION =
  "Tiếp tục dự án theo explicit project adapter và repository source of truth do project/Brain sở hữu. " +
  "Đọc các source-of-truth references được project adapter cung cấp; Supervisor không tự suy đoán business truth. " +
  "Trước mỗi thay đổi áp dụng QUESTION → DELETE → SIMPLIFY → ACCELERATE → AUTOMATE; không mở rộng ngoài critical path. " +
  "Tiếp tục đúng micro-task hiện tại, test/fix/regression/E2E, cập nhật project-owned state qua boundary được project cho phép rồi sang task kế tiếp nếu không cần Owner.";
...
export const OWNER_DECISION_RECONCILE_INSTRUCTION =
  ...
  "Nếu và chỉ nếu quyết định đó rõ ràng, cập nhật project-owned decision/contract/state qua boundary mà project adapter cho phép; sau đó tiếp tục critical path theo Five-Step. " +
...
export const HANDOFF_RECONCILE_INSTRUCTION =
  ...
  "Sau đó đối chiếu explicit project adapter và project-owned source-of-truth references. " +
