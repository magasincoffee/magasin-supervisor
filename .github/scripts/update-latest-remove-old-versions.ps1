...
if([string]::IsNullOrWhiteSpace($base)){throw 'Cannot resolve LOCALAPPDATA/USERPROFILE.'}
...
foreach($p in @($configFile,$registryFile,$installScript)){
  if(-not (Test-Path $p)){throw "Missing required canonical/install path: $p"}
}
...
foreach($laneName in @('lane-1','lane-2','lane-3')){
  $lane=$regBefore.lanes.$laneName
  if($lane){
    Write-Host ("PRE_"+$laneName+"_TASK="+[string]$lane.task_id)
    Write-Host ("PRE_"+$laneName+"_AWAITING="+[bool]$lane.awaiting_work)
  }
}

$sourcePanel=Get-Content (Join-Path $env:GITHUB_WORKSPACE 'windows\control-panel.ps1') -Raw -Encoding UTF8
foreach($marker in @('RESET READY','LÀM SẠCH TẤT CẢ DỰ ÁN','Request-RunnerRecovery','Request-LifecycleRecovery','for ($eventIndex = $events.Count - 1; $eventIndex -ge 0; $eventIndex--)')){
  if($sourcePanel -notmatch [regex]::Escape($marker)){
    throw "Latest checkout is missing required Control Panel marker: $marker"
  }
}
...
Get-CimInstance Win32_Process -Filter "Name='powershell.exe'" -ErrorAction SilentlyContinue |
  Where-Object {
    $_.CommandLine -and
    (
      $_.CommandLine -like '*control-panel.ps1*' -or
      $_.CommandLine -like '*run-supervisor.ps1*'
    )
  } |
  ForEach-Object {
...
Get-CimInstance Win32_Process -Filter "Name='node.exe'" -ErrorAction SilentlyContinue |
  Where-Object {
    $_.CommandLine -and
    $_.CommandLine -match '(supervisor-loop-cli|brain-worker-cli|three-lane-cli)\.mjs'
  } |
  ForEach-Object {
    Stop-Process -Id ([int]$_.ProcessId) -Force -ErrorAction SilentlyContinue
...
if(Test-Path $legacy){
  if([System.IO.Path]::GetFullPath($legacy).TrimEnd('\') -eq [System.IO.Path]::GetFullPath($canonical).TrimEnd('\')){
    throw 'Legacy path unexpectedly aliases canonical root.'
  }
...
foreach($marker in @('RESET READY','LÀM SẠCH TẤT CẢ DỰ ÁN','Request-RunnerRecovery','Request-LifecycleRecovery','for ($eventIndex = $events.Count - 1; $eventIndex -ge 0; $eventIndex--)')){
  if($installedPanel -notmatch [regex]::Escape($marker)){
    throw "Installed Control Panel missing required marker: $marker"
  }
}
Write-Host 'INSTALLED_PANEL_MARKERS=PASS'

if(-not (Test-Path $shortcutPath)){throw 'Unified Control Panel shortcut missing after install.'}
...
if($sc.Arguments -notlike "*$expectedPanel*"){throw 'Desktop shortcut does not target canonical current Control Panel.'}
if([System.IO.Path]::GetFullPath($sc.WorkingDirectory) -ne [System.IO.Path]::GetFullPath($canonical)){
  throw 'Desktop shortcut working directory is not canonical root.'
}

$fingerprintAfter=Get-TargetFingerprint $configFile
if($fingerprintBefore -ne $fingerprintAfter){throw 'Brain/Work/lane target fingerprint changed during update.'}
...
foreach($laneName in @('lane-1','lane-2','lane-3')){
  ...
  if($before -and $after){
    if([string]$before.task_id -ne [string]$after.task_id){throw "Task changed for $laneName during update."}
    if([bool]$before.awaiting_work -ne [bool]$after.awaiting_work){throw "awaiting_work changed for $laneName during update."}
  }
}
...
if($legacyProcesses.Count -ne 0){throw 'A process still references the deleted legacy root.'}
...
if(-not $panel){throw 'Current canonical Control Panel did not launch.'}
